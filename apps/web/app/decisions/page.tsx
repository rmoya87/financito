...
type DecisionOverview={
  ...
  prepayment_guardrail:{
    ...
    scenario:null|{reduced_payment:string;reduced_term_months:number;interest_saved_reduce_payment:string;interest_saved_reduce_term:string};
    missing:string[];
    notice:string;
  };
  signals:Signal[];
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-2">
        <Card>
          ...
          {overview.data.prepayment_guardrail.scenario&&<div className="mt-3 rounded-xl bg-[var(--brand-soft)] p-3 text-sm">
            <strong>Escenario ilustrativo con <Money value={overview.data.prepayment_guardrail.illustrative_extra_payment||'0'}/></strong>
            <div className="mt-2 grid gap-2 text-xs sm:grid-cols-2"><div>Cuota si reduces cuota: <strong><Money value={overview.data.prepayment_guardrail.scenario.reduced_payment}/></strong></div><div>Plazo si mantienes cuota: <strong>{overview.data.prepayment_guardrail.scenario.reduced_term_months} meses</strong></div><div>Ahorro de intereses reduciendo cuota: <strong><Money value={overview.data.prepayment_guardrail.scenario.interest_saved_reduce_payment}/></strong></div><div>Ahorro de intereses reduciendo plazo: <strong><Money value={overview.data.prepayment_guardrail.scenario.interest_saved_reduce_term}/></strong></div></div>
          </div>}
