  ...
  return <>
    ...
    <div className="mt-4 grid gap-4 xl:grid-cols-2">
      ...
      <VisualPanel title="Seguros y coberturas" description="Prima, franquicia y coberturas estructuradas a partir de tus pólizas." action={<Link className="text-xs underline" href="/insurance/">Ver todos</Link>}>
        <div className="mt-4 space-y-3">{insurance.data?.length?insurance.data.map(x=><div key={x.id} className="rounded-xl bg-[var(--surface-2)] p-3 text-sm"><div className="flex items-start justify-between gap-3"><strong>{x.insurance_type}</strong><strong><Money value={x.annual_premium}/>/año</strong></div><div className="mt-1 text-xs text-[var(--muted)]">Franquicia: {x.deductible===null?'sin dato':<Money value={x.deductible}/>}</div>{x.source_document_id&&<Link className="mt-2 inline-block text-xs underline" href={'/documents/?document='+encodeURIComponent(x.source_document_id)}>Ver póliza</Link>}</div>):<EmptyState>Aún no hay pólizas estructuradas. Añade la documentación para ver coberturas y condiciones aquí.</EmptyState>}</div>
      </VisualPanel>
    </div>
  </>;
}
