    ...
    {verdict.isLoading?<Loading/>:verdict.error?<ErrorState error={verdict.error}/>:data&&<>
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-2">
        <Card>
          ...
          <div className="mt-4 space-y-3">{data.policies.length?data.policies.map(p=><div key={p.id} role="button" tabIndex={0} aria-label={'Ver detalle del seguro '+(insuranceLabel[p.insurance_type]||p.insurance_type)} onClick={()=>setSelectedPolicyId(p.id)} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();setSelectedPolicyId(p.id)}}} className="cursor-pointer rounded-xl bg-[var(--surface-2)] p-4 outline-none focus:ring-2 focus:ring-[var(--brand)]">
