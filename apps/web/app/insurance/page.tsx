    ...
    {verdict.isLoading?<Loading/>:verdict.error?<ErrorState error={verdict.error}/>:data&&<>
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-2">
        ...
        <Card>
          ...
          {data.linked_products.length>0&&<div className="mt-4"><h3 className="text-sm font-semibold">Relaciones con hipoteca u otros productos</h3><div className="mt-2 space-y-2">{data.linked_products.map((x,i)=><div key={x.document_id+x.key+i} className="rounded-xl border border-[var(--border)] p-3 text-xs"><strong>{x.key}</strong><div className="mt-1">{typeof x.value==='string'?x.value:JSON.stringify(x.value)}</div><Link className="mt-1 inline-block underline" href={'/documents/?document='+encodeURIComponent(x.document_id)}>Ver evidencia{x.page?' · pág. '+x.page:''}</Link></div>)}</div>}
        </Card>
      </div>
