...
from ..domain.engines import CashFlowEngine
...
def answer(session:Session,question:str)->dict:
    ...
    tx=session.scalars(select(Transaction).where(Transaction.booking_date>=start,Transaction.booking_date<=today)).all()
    flow=CashFlowEngine.calculate((t.amount,t.is_internal_transfer) for t in tx)
