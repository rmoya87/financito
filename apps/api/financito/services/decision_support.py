...
from .contractual_costs import insurance_switching_context, prepayment_restrictions, resolve_prepayment_penalty
...
def _prepayment_guardrail(session: Session, context: dict, mortgage: Mortgage | None) -> dict:
    ...
    base_restrictions=prepayment_restrictions(session,mortgage)
    if base_restrictions["partial_prepayment_allowed"] is False:
        return {
            "status":"not_allowed",
            "protected_liquidity_reference_months":reference_months,
            "protected_liquidity_reference":str(protected),
            ...
            "missing":[],
            "notice":"La documentación confirmada indica que esta amortización parcial no está permitida; no se genera un escenario ejecutable.",
        }
    if base_restrictions["missing"] or base_restrictions["pending_review"]:
        return {
            "status":"needs_more_data",
            "protected_liquidity_reference_months":reference_months,
            "protected_liquidity_reference":str(protected),
            ...
            "scenario":None,
            "restrictions":base_restrictions,
            "missing":base_restrictions["missing"] + [
                f"prepayment_review:{row['key']}" for row in base_restrictions["pending_review"]
            ],
            "notice":"Antes de calcular una amortización fiable hay que confirmar el permiso y las condiciones materiales encontradas en el contrato.",
        }

    contract_max=Decimal(str(base_restrictions["effective_max_amount"] or mortgage.remaining_principal))
    extra=min(available,contract_max,mortgage.remaining_principal).quantize(Decimal("0.01"))
    contract_min=(
        None
        if base_restrictions["effective_min_amount"] is None
        else Decimal(str(base_restrictions["effective_min_amount"]))
    )
    if extra <= 0:
        return {
    ...
    if contract_min is not None and extra < contract_min:
        return {
            "status":"below_contract_minimum",
            "protected_liquidity_reference_months":reference_months,
            "protected_liquidity_reference":str(protected),
            "cash_above_reference":str(available),
            ...
            "restrictions":base_restrictions,
            "missing":[],
            "notice":f"El efectivo situado sobre el colchón de referencia no alcanza el mínimo contractual confirmado de {contract_min} €.",
        }

    restrictions=prepayment_restrictions(session,mortgage,extra)
    if not restrictions["calculation_ready"]:
        return {
            "status":"needs_more_data" if restrictions["missing"] or restrictions["pending_review"] else "amount_not_allowed",
            "protected_liquidity_reference_months":reference_months,
            "protected_liquidity_reference":str(protected),
            "cash_above_reference":str(available),
            "illustrative_extra_payment":str(extra),
            ...
            "missing":restrictions["missing"],
            "notice":"El importe no se usa para una conclusión hasta que cumpla todas las condiciones contractuales confirmadas.",
        }
    ...
    status="illustrative_with_manual_check" if restrictions["requires_manual_execution_check"] else "illustrative"
    return {
        "status": status,
        "protected_liquidity_reference_months": reference_months,
        "protected_liquidity_reference": str(protected),
        ...
        "scenario": {
            ...
            "reduced_payment": str(scenario.reduced_payment) if "payment" in options else None,
            "reduced_term_months": scenario.reduced_term_months if "term" in options else None,
            "interest_saved_reduce_payment": str(scenario.interest_saved_reduce_payment) if "payment" in options else None,
            "interest_saved_reduce_term": str(scenario.interest_saved_reduce_term) if "term" in options else None,
            "allowed_reduction_options":options,
        },
        "missing": [],
        "notice": (
            "El impacto financiero se calcula solo con datos confirmados. "
            + (
                "Antes de ejecutarlo queda una comprobación operativa indicada por el contrato."
                if restrictions["requires_manual_execution_check"]
                else "Las condiciones contractuales necesarias para este cálculo están confirmadas."
            )
        ),
    }
...
def decision_overview(session: Session, mortgage_id: str | None = None) -> dict:
    ...
    cards.append({
        ...
        "headline": (
            "Hay un escenario calculable sin consumir el colchón de liquidez de referencia."
            if prepayment["status"] in {"illustrative","illustrative_with_manual_check"}
        ...
        "next_action": (
            "Abre la simulación; Financito te mostrará también los límites y pasos contractuales antes de ejecutarla."
            if prepayment["status"] in {"illustrative","illustrative_with_manual_check"}
    ...
    return {
        ...
        "rules": [
            ...
            "Un coste, penalización, permiso o límite contractual material desconocido bloquea una conclusión favorable.",
