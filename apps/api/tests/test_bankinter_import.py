...
def _bankinter_xlsx()->bytes:
    ...
    ws.append(["21/09/2036","PENDIENTE COMERCIO A",-13,"EUR"])
    ws.append(["21/09/2036","PENDIENTE COMERCIO B",-5.10,"EUR"])
    ws.append([])
    ws.append(["MOVIMIENTOS"])
    ws.append(["Concepto","-"])
    ws.append(["Fecha","20/03/2036 - 20/09/2036"])
    ...
    ws.append(["21/09/2036","19/09/2036","PAGO BIZUM DE PERSONA",11,42124.35,"EUR"])
    ws.append(["17/09/2036","16/09/2036","MERCADONA",-114.26,42124.14,"EUR"])
    ws.append(["17/09/2036","17/09/2036","RECIBO AGUA",-21.70,42238.40,"EUR"])
    out=BytesIO()
    wb.save(out)
    return out.getvalue()
...
def test_bankinter_xlsx_uses_booked_ledger_not_pending_section():
    with SessionLocal() as db:
        ...
        assert bizum.booking_date.isoformat()=="2036-09-19"
        assert bizum.amount==Decimal("11")
        flow=cash_flow(db,date(2036,9,14),date(2036,9,19))
